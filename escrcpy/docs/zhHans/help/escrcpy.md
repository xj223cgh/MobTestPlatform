...
1. 设备输入法准备：安装支持物理键盘的输入法（推荐微信输入法）并完成设置。
2. 启动镜像：点击Escrcpy中的`开始镜像`。验证：设备的`设置` → `系统` → `语言与输入`中应显示`物理键盘`和`屏幕键盘`选项。
3. 设备输入设置：在`屏幕键盘`设置中启用微信输入法。在`物理键盘`设置中配置键盘布局与电脑键盘一致（仅需设置一次）。
4. 电脑输入准备：将输入模式设置为英文（重要）。
5. 切换输入语言：使用`Ctrl` + `Shift`切换中英文。
6. 开始使用。
...
1. 进入`设备列表`页面重试启用镜像。
2. 确保已下载安装最新版`Escrcpy`。
3. 按`Ctrl` + `Shift` + `I`打开开发者工具检查报错信息。
4. 若有报错，截图并提交至[反馈Issues](https://github.com/viarotel-org/escrcpy/issues)页面。
