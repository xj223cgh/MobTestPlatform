...
1. Device Input Method Preparation: Install an input method that supports physical keyboards (WeChat Input Method is recommended) and complete the setup.
2. Start Mirroring: Click `Start Mirroring` in Escrcpy. Verification: The device’s `Settings` → `System` → `Language & Input` should display options for `Physical Keyboard` and `On-screen Keyboard`.
3. Device Input Settings: Enable WeChat Input Method in the `On-screen Keyboard` settings. Configure the keyboard layout in the `Physical Keyboard` settings to match the computer keyboard (only needs to be done once).
4. Computer Input Preparation: Set the input mode to English (important).
5. Switch Input Language: Use `Ctrl` + `Shift` to switch between English and Chinese.
6. Start Using.
...
### Failed to get device list or error when starting mirroring

> This is generally caused by an incorrect path for `Adb` or `Scrcpy`, you can try the following solutions:

1. Go to the `Device List` page and try enabling mirroring again.
2. Make sure you have downloaded and installed the latest version of `Escrcpy`.
3. Press `Ctrl` + `Shift` + `I` to open the developer tools and check for any error messages.
4. If there are errors, take a screenshot and submit your issue with the screenshot on the [Feedback Issues](https://github.com/viarotel-org/escrcpy/issues) page.
