        ...
        >
          <div class="flex items-center gap-2">
            <el-input
              ...
              class="flex-1"
            />
            <el-button @click="selectFile('install')">
              {{ $t('common.select') }}
            </el-button>
          </div>
        </ele-form-item-col>
        ...
        >
          <div class="flex items-center gap-2">
            <el-input
              ...
              class="flex-1"
            />
            <el-button @click="selectFile('shell')">
              {{ $t('common.select') }}
            </el-button>
          </div>
        </ele-form-item-col>
...
async function selectFile(type) {
  try {
    ...
    const files = await window.electron.ipcRenderer.invoke('show-open-dialog', options)
    if (files && files.length > 0) {
      model.value.extra = files.join('; ')
    }
  } catch (error) {
