...
| 数据类型 | 说明 |
|----------|------|
| **用户** | 特殊账号：Lethe（超级管理员）、Manager（管理员）、Tester（测试人员）、Admin（普通成员）；测试用户：林裕铭、周瑾、陈国慧、林森、张俊浩、王灏然、储宏丽。密码统一 `123321` |
| **业务数据** | **用例与项目**：6 个迭代、16 条需求、13 个功能模块用例集（认证、看板、项目、用例、评审、AI 生成、专项）、评审、任务、设备、报告；**任务与设备**：3 个迭代、6 条需求、6+ 个用例集（测试任务、手工执行、报告、设备管理、系统管理、消息中心）、任务与报告 |
