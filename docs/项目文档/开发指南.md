...
│ - Pinia 状态管理     │     │ - Flask-Login 认证    │     │ - 27 张业务表     │
...
│   │   ├── routes/             # 路由蓝图（约 19 个 .py）
│   │   ├── services/           # 业务服务（邮件、通知等）
│   │   ├── constants/          # 常量（如 permissions.py 中权限码与 DEFAULT_ROLE_PERMISSIONS）
...
│   │   └── views/              # 页面视图（含 help/HelpCenter、notification 等）
...
### 数据库表一览（与 `database/03_create_tables.py` 一致，当前约 27 张表）
...
| `agents` | 本机 Agent 注册与心跳 |
| `user_agent_bindings` | 用户与本机 Agent 绑定（一用户一台） |
| `agent_binding_codes` | Agent 绑定码（短期有效） |

> **角色默认权限**：`role_permissions` 表无记录时，各角色初始勾选由 **`backend/app/constants/permissions.py`** 中 **`DEFAULT_ROLE_PERMISSIONS`** 决定；详见根目录 [`database/README.md`](../../database/README.md) 第 4.1 节。
...
### 路由蓝图一览（`backend/app/routes/`，当前 19 个）
...
| `mindmap.py` | `/api/mindmap` | 脑图数据与版本 |
...
| `agent_routes.py` | `/api/agent` | 本机 Agent 注册、心跳、绑定与下载 |
