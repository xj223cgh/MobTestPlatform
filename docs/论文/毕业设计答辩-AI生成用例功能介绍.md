...
实现上采用 **System + User** 双消息：

- **System**：强调严格依据文档、不编造、按功能点**完整覆盖**、**只输出 JSON**、不要代码块标记或废话——同时服务**贴合业务**与**少而全**。  
- **User**：嵌入完整需求正文，并规定 JSON 字段：`case_name`、`case_description`、`priority`、`preconditions`、`steps`、`expected_result`、`test_data` 等；要求 P0～P4、正常/异常/边界、**步骤可执行、预期可验证**——直接服务**清晰准确**；条数上按文档体量给**建议区间**，引导**少而全**而非无脑堆量。

**答辩金句**：不是「让模型随便写一段话」，而是**用结构化输出 + 覆盖与条数约束**，把模型拉向「贴业务、省条数、写得清」。
...
|----------|----------------|----------------|----------------|
| **贴合业务** | 步骤、预期对准真实功能与需求，少编造 | 提示词要求**严格依据文档**、步骤与预期**有据可依**；**低温**减少脱离文档的发挥 | `build_test_case_prompt`、`SYSTEM_ROLE_CONTENT`、`AI_TEMPERATURE` |
| **少而全** | 条数精炼，同时覆盖主流程与关键异常/边界 | 提示词要求按功能点**完整覆盖**；`_suggest_case_count` 按文档长度给**建议条数区间**，引导体量匹配、少重复堆砌 | `_suggest_case_count`、User 提示中的数量与覆盖要求 |
| **清晰准确** | 名称、步骤、预期写得可执行、可验证 | 固定 **JSON 字段**（名称、前置、步骤、预期、数据等）；明确要求**可执行、可验证** | `build_test_case_prompt` 中的输出格式与质量要求 |
...
| 方向 | 主要加强哪一维 |
|------|----------------|
| **Few-shot**（贴标准范例） | 清晰准确 + 贴合业务（风格与术语对齐） |
| **RAG**（检索需求/历史用例/接口说明） | 贴合业务 + 少而全（少漏测） |
| **功能 / 接口分流** + OpenAPI | 贴合业务（接口不被散文带偏） |
| **后置校验、相似度去重** | 清晰准确 + 少而全（控重复条数） |
| **分段生成再合并** | 少而全（长文档不漏中间功能点） |
| **固定需求集评测**（人工修改量、遗漏率） | 用数据验证三条是否同时变好 |
...
> 本系统在测试用例管理里增加了 AI 生成功能：用户针对单个需求粘贴或上传文档，后端创建异步任务，调用 OpenAI 兼容的大模型接口。优化上我按**效果好**三条来设计：**贴合业务**靠提示词要求有据可依、低温少胡编；**少而全**靠覆盖要求和建议条数随文档体量变化；**清晰准确**靠固定 JSON 字段和可执行、可验证的步骤与预期。另外用动态输出长度、解析剥离代码块等保证长文档也能完整落库。生成结果写入数据库并编号，前端轮询进度。效果还依赖需求是否写清、是否按单需求生成；后续可做 RAG、去重、接口分流等，仍围绕这三条加强。以上是实现思路与优化要点。
...
| `docs/方案/AI用例生成配置说明.md` | 环境变量、流程图、常见问题 |
| `backend/app/routes/ai_tasks.py` | 提示词、调用、解析、落库、动态 token 的核心实现 |
