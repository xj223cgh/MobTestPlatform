...
export const generateTestCaseService = async (params) => {
  try {
    ...
    let documentContent = "";
    if (params.file) {
      console.log("[AI生成用例] 开始解析文件:", params.file.name, params.file.type);
      documentContent = await parseDocument(params.file);
      console.log("[AI生成用例] 文档解析完成，内容长度:", documentContent.length);
    } else {
    ...
    function estimateTokenCount(text) {
      ...
      const punctuation = (text.match(/[^a-zA-Z0-9\u4e00-\u9fa5\s]/g) || []).length;
    ...
    console.log("[AI生成用例] 提示词构建完成，长度:", prompt.length, " 估算token数:", promptTokenCount);
    ...
    console.log("[AI生成用例] AI返回内容预览:", aiResponse.substring(0, 200) + (aiResponse.length > 200 ? "..." : ""));
    ...
    const formattedCases = aiGeneratedCases.map((caseItem, index) => {
      console.log(`[AI生成用例] 格式化第${index + 1}个用例:`, caseItem.case_name || "未命名用例");
      
      // 确保优先级为有效值
      const validPriorities = ['P0', 'P1', 'P2', 'P3', 'P4'];
      const priority = validPriorities.includes(caseItem.priority) ? caseItem.priority : 'P1';
      
      // 确保状态为有效值
      const validStatuses = ['', 'pass', 'fail', 'blocked', 'not_applicable'];
      const status = validStatuses.includes(caseItem.status) ? caseItem.status : '';
      ...
      const preconditions = caseItem.preconditions || '1. 系统正常运行\n2. 网络连接正常';
      const expectedResult = caseItem.expected_result || '系统按照预期执行，无异常报错';
      const testData = caseItem.test_data || '无特殊测试数据';
      const caseDescription = caseItem.case_description || caseItem.description || `${caseName} - 自动生成的功能测试用例`;
      ...
      console.log(`[AI生成用例] 第${index + 1}个用例格式化完成，字段完整性检查:`, {
    ...
    console.log("[AI生成用例] 所有用例格式化完成，共", formattedCases.length, "个用例");
...
export const saveGeneratedCases = async (cases, suiteId, params) => {
  try {
    ...
    try {
      ...
      // 如果是文件夹类型，创建一个新的用例集
        if (suiteDetail?.data?.type === 'folder') {
          ...
          const suiteName = params.suite_name || `${params.requirementName || 'AI生成用例集'}_${new Date().toLocaleDateString().replace(/\//g, '-')}`;
          ...
          const newSuiteData = {
            ...
            project_id: params.projectId,
            parent_id: suiteId,
            description: params.description || `AI生成的用例集，包含${cases.length}个测试用例`
          };
    ...
    try {
      ...
      console.log("[AI保存用例] 获取现有用例成功，返回数据:", response?.data?.total || 0, "个用例");
    ...
    const projectShortName = (params.projectName?.replace(/[^a-zA-Z0-9]/g, "").toUpperCase() || "PROJ").slice(0, 3);
    
    // 迭代版本：从迭代名称中提取版本号，如 V1.0.0 → 1.0.0
    const iterationVersion = params.iterationName?.match(/\d+\.\d+\.\d+/)?.[0] || "1.0.0";
    
    // 需求缩写：取需求名称首字母或关键部分，最多3个字符
    const requirementShortName = (params.requirementName?.replace(/[^a-zA-Z0-9]/g, "").toUpperCase() || "REQ").slice(0, 3);
    ...
    // 3. 批量保存测试用例到数据库
    console.log("[AI保存用例] 步骤3: 开始批量保存测试用例，共", cases.length, "个用例");
    const savedCases = [];
    for (let i = 0; i < cases.length; i++) {
      ...
      const caseName = caseItem.case_name || caseItem.case_description?.substring(0, 50) || `测试用例_${caseNumber}`;
      
      console.log(`[AI保存用例] 生成用例编号: ${caseNumber}, 名称: ${caseName}`);
      ...
      console.log(`[AI保存用例] 第${i + 1}个用例保存成功，返回ID:`, savedCase?.data?.id || '未知');
      
      savedCases.push(savedCase.data);
    }
    
    console.log("[AI保存用例] 所有用例保存完成，共保存", savedCases.length, "个用例");
