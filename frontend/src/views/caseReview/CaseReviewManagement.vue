              ...
              >
                <template #default="scope">
                  <el-button 
                    type="primary" 
                    size="small"
                    @click.stop="(scope.row.status === 'completed' || scope.row.status === 'rejected') ? handleViewDetail(scope.row) : handleReview(scope.row)"
                  >
                    {{ scope.row.status === 'completed' ? '查看评审' : 
                      scope.row.status === 'in_review' ? '继续评审' : 
                      scope.row.status === 'rejected' ? '查看评审' : '开始评审' }}
                  </el-button>
                </template>
              </el-table-column>
            </el-table>
          ...
          <div class="review-section">
                        ...
                        >
                          <template #default="{ node, data }">
                            ...
                            >
                              <el-icon
                                class="node-icon"
                                @click.stop="data.type === 'suite' && handleSuiteSelect(data)"
                              >
            ...
            >
              ...
              >
                <template #default="scope">
                  <el-tag 
                    :type="getStatusTagType(scope.row.status, scope.row.history_type)"
                    size="small"
                  >
      ...
      >
        ...
        <div class="dialog-section">
          ...
          >
            ...
            >
              <el-tag :type="getStatusTagType(currentReviewTask?.status, currentReviewTask?.history_type)">
                {{ getStatusText(currentReviewTask?.status, currentReviewTask?.history_type) }}
              </el-tag>
            </el-descriptions-item>
          </el-descriptions>
        </div>
        ...
        <div class="dialog-section">
          ...
          >
            ...
            >
              <template #default="scope">
                <div class="text-with-newlines">
                  {{ scope.row.preconditions || scope.row.test_case?.preconditions || '-' }}
                </div>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template #default="scope">
                <div class="text-with-newlines">
                  {{ scope.row.expected_result || scope.row.test_case?.expected_result || '-' }}
                </div>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template #default="scope">
                <div class="text-with-newlines">
                  {{ scope.row.actual_result || scope.row.test_case?.actual_result || '-' }}
                </div>
              </template>
            </el-table-column>
            <el-table-column
        ...
        <span class="dialog-footer">
          ...
          <template v-if="!currentReviewTask?.version">
            ...
            <template v-if="isReviewer">
              <!-- 如果评审未完成，显示完成评审按钮 -->
              <el-button 
                v-if="currentReviewTask && currentReviewTask.status !== 'completed' && currentReviewTask.status !== 'rejected'"
                type="primary"
              ...
              <!-- 如果评审已完成，显示重新评审按钮 -->
              <el-button 
                v-else-if="currentReviewTask && currentReviewTask.status === 'completed'"
                type="warning"
              ...
              <el-button 
                v-if="currentReviewTask && currentReviewTask.status !== 'pending' && currentReviewTask.status !== 'rejected'"
                type="danger"
            ...
            <el-button 
              v-if="isInitiator && currentReviewTask && currentReviewTask.status === 'rejected'"
              type="warning"
...
const handleGetReviewHistory = async () => {
  ...
  try {
    const response = await reviewApi.getSuiteReviewStatus(selectedSuiteId.value)
...
const handleCompleteReview = async () => {
  ...
  try {
    ...
    for (const currentReview of caseReviews.value) {
      // 找到对应的原始评审数据
      const originalReview = originalCaseReviews.value.find(orig => orig.id === currentReview.id)
      
      // 如果找到原始数据并且有修改，添加到修改列表中
      if (originalReview) {
        // 检查评审状态或评审意见是否有修改
        if (currentReview.review_status !== originalReview.review_status || 
...
const handleRestartReview = async () => {
  ...
  await ElMessageBox.confirm('确定要重新评审该任务吗？此操作将允许您修改评审结果。', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  })
...
const handleReinitiateReview = async () => {
  ...
  await ElMessageBox.confirm('确定重新发起评审进入评审状态，并保留用例评审结果吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  })
...
const handleRejectReview = async () => {
  ...
  await ElMessageBox.confirm('确定要打回评审吗？此操作将保存当前评审编辑并重置评审状态。', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  })
  ...
  try {
    ...
    ElMessage.error('打回评审失败: ' + (error.response?.data?.message || error.message))
  } finally {
    loading.value.updateReview = false
  }
}
