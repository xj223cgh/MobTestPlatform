<template>
  ...
  >
    ...
    <div v-loading="loading" element-loading-text="加载中" class="p-2" :class="{ '!h-auto': !connectFlag }">
      <div v-if="connectFlag" class="flex-none overflow-hidden rounded-lg shadow bg-gray-200 dark:bg-gray-700 object-contain cursor-pointer flex items-center justify-center" style="height: 600px;">
        <img v-if="deviceInfo.screencap" :src="deviceInfo.screencap" class="w-full h-full object-contain" alt="设备截图" @click="handlePreview" />
        <div v-else class="text-center p-1 text-gray-500">
          <el-icon class="text-xl mb-0.5"><Picture /></el-icon>
          <p class="text-xs">无法获取截图</p>
        </div>
      </div>

      <div class="overflow-auto mt-1" style="max-height: 40px;">
        <el-descriptions border :column="1" class="el-descriptions--custom text-sm">
          ...
          <template v-if="deviceInfo.battery">
            <el-descriptions-item label="电池电量">
              {{ deviceInfo.battery.batteryPercentage !== null ? `${deviceInfo.battery.batteryPercentage}%` : '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="是否充电">
              {{ deviceInfo.battery.isCharging !== null ? (deviceInfo.battery.isCharging ? '是' : '否') : '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="温度">
              {{ deviceInfo.battery.temperatureCelsius !== null ? `${deviceInfo.battery.temperatureCelsius}℃` : '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="电源来源">
              {{ deviceInfo.battery.powerSource !== null ? deviceInfo.battery.powerSource : '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="电压">
              {{ deviceInfo.battery.voltageV !== null ? `${deviceInfo.battery.voltageV}v` : '-' }}
            </el-descriptions-item>
          </template>
        </el-descriptions>
      </div>
...
async function getScreencap() {
  try {
    ...
    const response = await deviceApi.executeAdbCommand(`-s ${props.device.id} shell screencap -p | base64`, {
      isHovering: isHovering.value
    })
    // 处理base64数据，去除换行符和可能的错误信息
    const rawOutput = response.data.stdout
    // 只保留base64部分，去除可能的错误信息
    const base64Data = rawOutput.replace(/\n/g, '').replace(/^.*?base64,?/i, '')
    
    // 验证base64数据是否有效
    if (base64Data && base64Data.length > 0 && /^[A-Za-z0-9+/=]+$/.test(base64Data)) {
...
async function getBattery() {
  try {
    ...
    const response = await deviceApi.executeAdbCommand(`-s ${props.device.id} shell dumpsys battery`, {
      isHovering: isHovering.value
    })
