...
// 重试请求函数
const retryRequest = async (fn, retries = aiConfig.retryCount, requestName = "未知请求") => {
  try {
    ...
    if (retries <= 0) {
      console.error(`[AI API] ${requestName}所有重试均失败:`, error.response?.data || error.message || error);
      throw error;
    }
...
export const generateTestCases = async (prompt) => {
  return retryRequest(async () => {
    ...
    console.log("[AI API] 生成测试用例响应状态:", response.status, "耗时:", response.config?.meta?.requestStartTime ? `${Date.now() - response.config.meta.requestStartTime}ms` : '未知');
    console.log("[AI API] 响应数据包含choices:", response.data?.choices?.length > 0);
    
    return response.data;
  }, aiConfig.retryCount, "生成测试用例");
};
...
export const generateSingleTestCase = async (prompt) => {
  return retryRequest(async () => {
    ...
    console.log("[AI API] 生成单个测试用例响应状态:", response.status, "耗时:", response.config?.meta?.requestStartTime ? `${Date.now() - response.config.meta?.requestStartTime}ms` : '未知');
    console.log("[AI API] 响应数据包含choices:", response.data?.choices?.length > 0);
    
    return response.data;
  }, aiConfig.retryCount, "生成单个测试用例");
};
...
aiRequest.interceptors.request.use(config => {
  ...
  console.log("[AI API] 请求开始:", config.url, "方法:", config.method.toUpperCase());
  return config;
});
...
aiRequest.interceptors.response.use(response => {
  ...
  console.log("[AI API] 请求结束:", response.config.url, "耗时:", duration, "ms");
  return response;
});
