...
在「访问平台的那台电脑」上运行，通过 Socket.IO 连接平台后端，在本机执行 adb/scrcpy，实现设备列表、投屏、脚本执行等设备管理能力。**adb 与 scrcpy 已封装进 exe，用户直接运行即可，无需单独安装。**
...
4. 绑定成功后，设备管理页显示「本机 Agent 已连接」，设备列表、投屏、脚本执行等均在本机执行。

详细流程（含部署、再次访问、清理）见 **AGENT_FLOW.md**。用户无需单独安装 adb、scrcpy。

## Agent 与平台通信

- **通信协议**：Socket.IO（基于 WebSocket），Agent 主动连接平台后端。
- **默认地址**：`http://127.0.0.1:5000`，平台在服务器上时需改为服务器地址。
- **心跳**：Agent 定期上报在线状态，后端据此判断设备列表来源。
- **本地监听**：`127.0.0.1:8765`（本地回环），用于浏览器「一键绑定」和状态检测。

## ADB 版本冲突处理

Agent 内置了 adb 与 scrcpy，启动时会自动执行 `adb kill-server` + `adb start-server`，用内置版本接管本机的 ADB Server。这样即使用户电脑上已安装了其他版本的 ADB，也不会出现多版本 ADB Server 互相冲突导致设备列表闪烁或命令超时的问题。

- **ADB 查找优先级**：环境变量 `ADB_PATH`/`ADB` → 内置 `bin/adb.exe` → exe 同级目录 → 系统 PATH。
- **Agent 退出后**：内置 ADB Server 随之停止，用户如需使用自己的 ADB，执行 `adb start-server` 即可恢复。
...
| **网络** | 仅监听本机 `127.0.0.1:8765`（本地回环），用于平台页一键绑定和状态检测；并主动连接您配置的平台地址（Socket.IO）。不开放公网端口。 |
...
- 平台访问与 Agent 流程：[平台访问与Agent流程说明.md](../docs/方案/平台访问与Agent流程说明.md)
- 内网访问：[内网访问说明.md](../docs/方案/内网访问说明.md)
